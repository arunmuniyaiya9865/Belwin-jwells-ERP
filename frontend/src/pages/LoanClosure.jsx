...
const LoanClosure = () => {
    ...
    const handleSearch = async (e) => {
        ...
        setIsLoading(true);
        try {
            const res = await getClosureDetails(searchQuery.trim().toUpperCase());
            if (res.success) {
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 print:hidden">
                <form onSubmit={handleSearch} className="flex gap-3">
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="px-6 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-400"
                    >
