...
<<<<<<< HEAD
  ...
  const fetchNextExpenseId = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/expenses/next-id');
      const data = await response.json();
      if (response.ok && data.nextId) {
        setFormData(prev => ({ ...prev, expenseId: data.nextId }));
      }
    } catch (error) {
