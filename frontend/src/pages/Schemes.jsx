...
const Schemes = () => {
  ...
  const fetchSchemes = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/gold-schemes');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = isGold ? 'http://localhost:5000/api/gold-schemes' : 'http://localhost:5000/api/schemes';
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const isGold = selectedCategory === 'Bellwin Gold Loan';
      const endpoint = isGold ? `http://localhost:5000/api/gold-schemes/${id}` : `http://localhost:5000/api/schemes/${id}`;
