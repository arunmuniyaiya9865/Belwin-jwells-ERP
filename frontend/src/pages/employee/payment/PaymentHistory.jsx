...
import { Filter, Download, Printer, Eye, Calendar, User, FileText, MapPin, BadgeDollarSign, X } from 'lucide-react';
...
const PaymentHistory = () => {
  ...
  const handleSelectLoan = async (loanIdStr) => {
    ...
    setLoanSummary(null);
    try {
      const data = await getPaymentHistory(loanIdStr.toUpperCase());
      setLoanSummary(data);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="mb-4 bg-white border border-gray-200 rounded-xl shadow-sm p-4 shrink-0">
        ...
        {isSearchingPayments && (
           <div className="text-sm font-semibold text-gray-500 py-4">Fetching payments...</div>
        )}
          ...
          <div className="mt-4 grid grid-cols-6 gap-3">
            ...
            <div className="flex justify-end col-span-6 mt-2">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-gray-100 text-gray-600 text-sm font-semibold rounded-lg hover:bg-red-50 hover:text-red-600 transition"
              >
                <X className="w-3.5 h-3.5" /> Clear Loan Search
              </button>
            </div>
          </div>
        )}
