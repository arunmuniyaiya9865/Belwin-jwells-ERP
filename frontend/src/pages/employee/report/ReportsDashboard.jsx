...
const ReportsDashboard = () => {
  ...
  const getCardStyle = (idx) => {
    return { bg: 'from-emerald-900 to-emerald-950 border-emerald-800', text: 'text-emerald-200' };
  };
  
  const [activeReport, setActiveReport] = useState(
    REPORTS.find(r => r.id === currentPath) || REPORTS[0]
  );

  useEffect(() => {
    const report = REPORTS.find(r => r.id === currentPath);
    if (report && report.id !== activeReport.id) {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await api.get(`http://127.0.0.1:5000${endpoint}?${params.toString()}`);
  ...
  const processData = (id, data) => {
    ...
    switch (id) {
      ...
      case 'today-collection-report':
        result.cards = [
          { label: 'Total Collections Today', value: formatCurrency(data.summary?.totalCollectionAmount), highlight: true },
          { label: 'Principal Collection', value: formatCurrency(data.summary?.totalPrincipalCollection) },
          { label: 'Interest Collection', value: formatCurrency(data.summary?.totalInterestCollection) },
          { label: 'Customers Collected', value: data.summary?.totalCustomersCollected }
        ];
        result.columns = ['PAYMENT ID', 'DATE', 'LOAN ID', 'CUSTOMER NAME', 'TYPE', 'AMOUNT', 'PRINCIPAL', 'INTEREST', 'DOC CHARGE'];
        result.rows = (data.tableData || []).map(p => [
          p._id.toString().substring(0, 8), new Date(p.date).toLocaleDateString(), p.loanId, p.customerName || 'N/A',
          p.paymentMode, formatCurrency(p.totalAmount), formatCurrency(p.principalAmount), formatCurrency(p.interestAmount), formatCurrency(p.documentCharges)
        ]);
        break;
      ...
      case 'loan-report':
        ...
        result.columns = ['LOAN ID', 'CUSTOMER ID', 'NAME', 'START DATE', 'END DATE', 'AMOUNT', 'REMAINING', 'STATUS', 'CREATED'];
        result.rows = data.map(d => [
          d.loanId, d.customerId, d.customerName || 'N/A', new Date(d.loanStartDate || d.loanDate).toLocaleDateString(),
          d.loanEndDate ? new Date(d.loanEndDate).toLocaleDateString() : 'N/A',
      ...
      case 'loan-outstanding-report':
        ...
        result.columns = ['LOAN ID', 'CUSTOMER ID', 'NAME', 'START DATE', 'END DATE', 'AMOUNT', 'REMAINING', 'STATUS', 'INTEREST', 'DAYS'];
        result.rows = data.map(d => [
          d.loanId, d.customerId, d.customerName || 'N/A', new Date(d.loanStartDate || d.loanDate).toLocaleDateString(),
          d.loanEndDate ? new Date(d.loanEndDate).toLocaleDateString() : 'N/A',
      ...
      case 'interest-pending-report':
        ...
        result.columns = ['LOAN ID', 'CUSTOMER ID', 'NAME', 'START DATE', 'END DATE', 'AMOUNT', 'RATE', 'PAID INT', 'PENDING INT', 'STATUS'];
        result.rows = data.map(d => [
          d.loanId, d.customerId, d.customerName || 'N/A', new Date(d.loanStartDate || d.loanDate).toLocaleDateString(),
          d.loanEndDate ? new Date(d.loanEndDate).toLocaleDateString() : 'N/A',
      ...
      case 'datewise-pending-list':
        ...
        result.columns = ['LOAN ID', 'CUSTOMER ID', 'NAME', 'START DATE', 'END DATE', 'AMOUNT', 'PENDING PRINCIPAL', 'PENDING INTEREST', 'STATUS', 'PENDING DAYS'];
        result.rows = data.map(d => [
          d.loanId, d.customerId, d.customerName || 'N/A', new Date(d.loanStartDate || d.loanDate).toLocaleDateString(),
          d.loanEndDate ? new Date(d.loanEndDate).toLocaleDateString() : 'N/A',
      ...
      case 'closed-account-report':
        ...
        result.columns = ['LOAN ID', 'CUSTOMER ID', 'NAME', 'START DATE', 'END DATE', 'AMOUNT', 'SETTLEMENT', 'CLOSED DATE', 'STATUS'];
        result.rows = data.map(d => [
          d.loanId, d.customerId, d.customerName || 'N/A', new Date(d.loanStartDate || d.loanDate).toLocaleDateString(),
          d.loanEndDate ? new Date(d.loanEndDate).toLocaleDateString() : 'N/A',
      ...
      case 'auction-account':
        ...
        result.columns = ['LOAN ID', 'CUSTOMER ID', 'CUSTOMER NAME', 'START DATE', 'END DATE', 'LOAN AMOUNT', 'REMAINING AMOUNT', 'STATUS', 'PENDING DAYS'];
        result.rows = data.map(d => [
          d.loanId, d.customerId, d.customerName || 'N/A', new Date(d.loanStartDate || d.loanDate).toLocaleDateString(),
          d.loanEndDate ? new Date(d.loanEndDate).toLocaleDateString() : 'N/A',
                ...
                <div key={i}>
                  ...
                  <div className={`grid gap-4 ${sec.cards.length > 4 ? 'grid-cols-5' : `grid-cols-${sec.cards.length}`}`}>
                    {sec.cards.map((card, idx) => {
                      ...
                      return (
                        <div key={idx} className={`bg-gradient-to-br ${style.bg} p-4 rounded-xl border flex items-center justify-between shadow-lg`}>
                          <div>
                            <p className={`${style.text} text-sm font-medium mb-1`}>
                              {card.label}
                            </p>
