...
const TopUpLoan = () => {
  ...
  useEffect(() => {
    if (activeTab === 'reports') {
      const fetchTopUps = async () => {
        try {
          const res = await api.get('http://127.0.0.1:5000/api/topups');
  ...
  const fetchHistory = async (loanId) => {
    try {
      const res = await api.get(`http://127.0.0.1:5000/api/topups/loan/${loanId}`);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const res = await api.post('http://127.0.0.1:5000/api/topups', payload);
      toast.success('Top-up request saved successfully!');
      
      // Refresh loan details from backend (re-fetch current loan by calling its endpoint directly, since global search is cleared or might be for another purpose)
      const res2 = await api.get(`http://127.0.0.1:5000/api/loans/${selectedLoan.loanId}`);
      if (res2.data) {
          handleSelectLoan(res2.data);
      }
            ...
            {!selectedLoan ? (
              <div className="flex flex-col gap-4">
                ...
                {results.length === 0 && !isSearchingLoans && (
                  <div className="flex-1 flex items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 rounded-xl bg-white/50 min-h-[200px] mt-4">
              ...
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                <div className="flex justify-end">
                  <button type="button" onClick={() => { setSelectedLoan(null); clearSearch(); }} className="text-xs text-gray-500 hover:text-red-500 font-semibold bg-gray-100 px-3 py-1.5 rounded-lg transition-colors">
                    Change Loan
                  </button>
                </div>
