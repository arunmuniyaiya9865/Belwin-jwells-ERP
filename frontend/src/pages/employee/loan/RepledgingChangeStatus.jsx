...
import { Save, RefreshCcw, History, FileText, ClipboardCheck, ChevronDown } from 'lucide-react';
...
const RepledgingChangeStatus = () => {
  ...
  const handleSelectLoan = async (loanIdStr) => {
    ...
    try {
      ...
      const history = await getRepledgesByLoan(loanObj.loanId);
      setStatusHistory(history);
      toast.success(`Loan ${loanObj.loanId} loaded`);
    } catch (error) {
      toast.error(error.response?.data?.message || 'Error fetching loan history');
      setSelectedLoan(null);
      setStatusHistory([]);
    } finally {
            ...
            {!selectedLoan ? (
              <div className="flex flex-col gap-4">
                ...
                {results.length === 0 && !isSearchingLoans && (
                  <div className="flex-1 flex items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 rounded-xl bg-white/50 min-h-[200px] mt-4">
                    <p className="text-sm font-medium">Enter a Loan ID or Phone Number and search to manage repledging</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                <div className="flex justify-end">
                  <button onClick={() => { setSelectedLoan(null); clearSearch(); }} className="text-xs text-gray-500 hover:text-red-500 font-semibold bg-gray-100 px-3 py-1.5 rounded-lg transition-colors">
                    Change Loan
                  </button>
                </div>
