...
const EditLoan = () => {
  ...
  const populateLoan = (loan) => {
    ...
    setFormData({
      ...
      loanStartDate: formatDateString(loan.loanStartDate || loan.loanDate),
      loanEndDate: formatDateString(loan.loanEndDate),
      maturePeriod: loan.maturePeriod || '',
  ...
  const handleSearch = async (searchValue) => {
    ...
    if (data && data.count === 1) {
      populateLoan(data.results[0].loan);
    }
  };

  const handleSelectLoan = (loanIdStr) => {
    const loanObj = results.find(r => r.loan.loanId === loanIdStr)?.loan;
    if (loanObj) {
      populateLoan(loanObj);
    }
  };
  ...
  return (
    <div className="h-full w-full flex flex-col p-4 md:p-8">
      ...
      <div className="mb-4 shrink-0 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        {/* Search Bar */}
        {/* Search components moved below title */}
      ...
      <div className="flex-1 overflow-y-auto">
        {!hasSearched ? (
          <div className="mt-8">
            ...
            {results.length === 0 && !isSearchingLoans && (
              <div className="h-full w-full flex items-center justify-center pb-32 mt-12">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm font-semibold text-gray-400">Enter a MongoDB Loan ID or Phone Number above to view and edit details.</p>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === 'Receipt' && (
          <div className="w-full">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-12 gap-y-2 mb-6 pb-6 border-b border-erp-green-dark">
              ...
              <div>
                <div className={row}><span className={lbl}>Loan Start Date :</span><input type="date" name="loanStartDate" value={formData.loanStartDate} onChange={handleChange} className={inp} required /></div>
                <div className={row}><span className={lbl}>Loan End Date :</span><input type="date" name="loanEndDate" value={formData.loanEndDate} className={`${inp} bg-gray-100 cursor-not-allowed`} readOnly /></div>
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 mb-8">
              ...
              <button onClick={() => { setHasSearched(false); clearSearch(); }} className="px-6 py-2 bg-gradient-to-r from-red-700 to-red-900 text-white text-sm font-bold rounded shadow-md hover:from-red-800 hover:to-red-950 transition-all">Cancel</button>
